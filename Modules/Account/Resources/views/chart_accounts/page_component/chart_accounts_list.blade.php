                    ...
                    <div class="dropdown CRM_dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right"
                            ...
                            @if ($charAccount->id > "22")
                                <a href="#" data-toggle="modal" data-target="#ChartAccount_Edit" class="dropdown-item edit_chart_account" data-value="{{$charAccount->id}}" type="button">{{ __('common.Edit') }}</a>
                                <a onclick="confirm_modal('{{route('char_accounts.destroy',$charAccount->id)}}');" class="dropdown-item ">{{__('common.Delete')}}</a>
                            @else
                                <a href="#" data-toggle="modal" data-target="#RenameAccount" class="dropdown-item rename_account" onclick="get_data_modal({{$charAccount}})" type="button">{{ __('common.Edit') }}</a>
                            @endif
                        </div>
                    </div>
                    <!-- shortby  -->
