            ...
            @if(permissionCheck('showroom.index'))
                <li>
                    <a href="{{ route('showroom.index') }}"
                       class="{{ spn_active_link(['showroom.index', 'showroom.show']) }}">{{ __('inventory.Branch') }}</a>
                </li>
            @endif
            @if(permissionCheck('warehouse.index'))
                <li>
                    <a href="{{ route('warehouse.index') }}"
                       class="{{ spn_active_link(['warehouse.index', 'warehouse.show']) }}">{{ __('inventory.Warehouse') }}</a>
                </li>
            @endif
        </ul>
    </li>
            ...
            @if (permissionCheck('departments.index'))
                <li>
                    <a href="{{ route('departments.index') }}"
                       class="{{request()->is('hr/departments') ? 'active' : ''}}">{{ __('department.Department') }}</a>
                </li>
            @endif
            @if (permissionCheck('attendances.index'))
                <li>
                    <a href="{{ route('attendances.index') }}"
                       class="{{request()->is('attendance/hr/attendance') ? 'active' : ''}}">{{ __('attendance.Attendance') }}</a>
                </li>
            @endif
            @if (permissionCheck('attendance_report.index'))
                <li>
                    <a href="{{ route('attendance_report.index') }}"
                       class="{{ request()->is('attendance/hr/attendance/*') ? 'active' : ''}}">{{ __('attendance.Attendance Report') }}</a>
                </li>
            @endif
            @if (permissionCheck('events'))
                <li>
                    <a href="{{ route('events.index') }}"
                       class="{{request()->is('events') || request()->is('events/*') ? 'active' : ''}}">{{ __('event.Event') }}</a>
                </li>
            @endif
            ...
            @if (permissionCheck('payroll.index'))
                <li>
                    <a href="{{ route('payroll.index') }}"
                       class="{{ spn_active_link(['staff_search_for_payroll', 'payroll.index']) }}">{{ __('payroll.Payroll') }}</a>
                </li>
            @endif
            @if (permissionCheck('payroll_reports.index'))
                <li>
                    <a href="{{ route('payroll_reports.index') }}"
                       class="{{ spn_active_link(['payroll_reports.index', 'payroll_reports.search']) }}">{{ __('payroll.Payroll Reports') }}</a>
                </li>
            @endif
            @if (permissionCheck('apply_loans'))
                <li>
                    <a href="{{ route('apply_loans.index') }}"
                       class="{{request()->is('hr/apply-loans') ? 'active' : ''}}">{{ __('common.Loan Apply') }}</a>
                </li>
            @endif
            @if (permissionCheck('apply_loans.history'))
                <li>
                    <a href="{{ route('apply_loans.history') }}"
                       class="{{request()->is('hr/apply-loans/history') ? 'active' : ''}}">{{ __('setup.Loan History') }}</a>
                </li>
            @endif
            @if (permissionCheck('apply_loans.loan_approval_index'))
                <li>
                    <a href="{{ route('apply_loans.loan_approval_index') }}"
                       class="{{request()->is('hr/loan-approval') ? 'active' : ''}}">{{ __('common.Loan Approval') }}</a>
                </li>
            @endif
        </ul>
    </li>
