...
class StaffController extends Controller
{
            ...
            if ($request->password) {
                try {
                    ...
                    if ($staff and $staff->phone) {
                        ...
                        $this->sendNotification($staff, $staff->user->email, $subject , $content, $number, $message,$users);
                    }
                    DB::commit();
    ...
    public function update(StaffUpdateRequest $request, $id)
    {
        ...
        try {
            ...
            $users=User::where('id',$id)->where('id','!=',auth()->user()->id)
                        ->where('is_active',1)
                        ->get(['id','role_id']);
    ...
    public function destroy($id)
    {
        if ($id == 1 or auth()->id() == $id){
            Toastr::error(__('common.You dont have permission to delete this user'));
            return back();
        }
        try {
